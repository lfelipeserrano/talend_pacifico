...
package pacifico.saleordertemplate_dailyjob_0_1;
...
/**
 * Job: saleOrderTemplate_DailyJob Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class saleOrderTemplate_DailyJob implements TalendJob {
	...
	private final String jobName = "saleOrderTemplate_DailyJob";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					saleOrderTemplate_DailyJob.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(saleOrderTemplate_DailyJob.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
		final static byte[] commonByteArrayLock_PACIFICO_saleOrderTemplate_DailyJob = new byte[0];
		static byte[] commonByteArray_PACIFICO_saleOrderTemplate_DailyJob = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PACIFICO_saleOrderTemplate_DailyJob.length) {
					if (length < 1024 && commonByteArray_PACIFICO_saleOrderTemplate_DailyJob.length == 0) {
						commonByteArray_PACIFICO_saleOrderTemplate_DailyJob = new byte[1024];
					} else {
						commonByteArray_PACIFICO_saleOrderTemplate_DailyJob = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PACIFICO_saleOrderTemplate_DailyJob, 0, length);
				strReturn = new String(commonByteArray_PACIFICO_saleOrderTemplate_DailyJob, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PACIFICO_saleOrderTemplate_DailyJob, 0, length);
				strReturn = new String(commonByteArray_PACIFICO_saleOrderTemplate_DailyJob, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PACIFICO_saleOrderTemplate_DailyJob) {
...
///////////////////////    			
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:OJEzG4upnp1L5tVzJBGKN0HmQzcIyPQSPMetESodOqLYz43XVQ==");
				...
				String dbquery_tDBInput_1 = "SELECT * FROM \"sale_order_template\" where create_date between '"
						+ context.fechaInicio + "' and '" + context.fechaFin + "' OR  write_date between '"
						+ context.fechaInicio + "' and '" + context.fechaFin + "'";
...
///////////////////////		
						...
						if (row2.note != null) { //
							row_tLogRow_1[0] = String.valueOf(row2.note);

						} //

						if (row2.number_of_days != null) { //
							row_tLogRow_1[1] = String.valueOf(row2.number_of_days);

						} //

						if (row2.require_signature != null) { //
							row_tLogRow_1[2] = String.valueOf(row2.require_signature);

						} //

						if (row2.require_payment != null) { //
							row_tLogRow_1[3] = String.valueOf(row2.require_payment);

						} //

						if (row2.mail_template_id != null) { //
							row_tLogRow_1[4] = String.valueOf(row2.mail_template_id);

						} //

						if (row2.active != null) { //
							row_tLogRow_1[5] = String.valueOf(row2.active);

						} //

						if (row2.company_id != null) { //
							row_tLogRow_1[6] = String.valueOf(row2.company_id);

						} //

						row_tLogRow_1[7] = String.valueOf(row2.id);

						if (row2.name != null) { //
							row_tLogRow_1[8] = String.valueOf(row2.name);

						} //

						if (row2.create_uid != null) { //
							row_tLogRow_1[9] = String.valueOf(row2.create_uid);

						} //

						if (row2.create_date != null) { //
							row_tLogRow_1[10] = FormatterUtils.format_Date(row2.create_date, "dd-MM-yyyy");

						} //

						if (row2.write_uid != null) { //
							row_tLogRow_1[11] = String.valueOf(row2.write_uid);

						} //

						if (row2.write_date != null) { //
							row_tLogRow_1[12] = FormatterUtils.format_Date(row2.write_date, "dd-MM-yyyy");
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final saleOrderTemplate_DailyJob saleOrderTemplate_DailyJobClass = new saleOrderTemplate_DailyJob();

		int exitCode = saleOrderTemplate_DailyJobClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = saleOrderTemplate_DailyJob.class.getClassLoader().getResourceAsStream(
					"pacifico/saleordertemplate_dailyjob_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = saleOrderTemplate_DailyJob.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : saleOrderTemplate_DailyJob");
		}
...
/************************************************************************************************
 * 114712 characters generated by Talend Open Studio for Data Integration on the
 * January 26, 2023 at 11:49:21 PM CST
 ************************************************************************************************/