				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(getSaleOrderTemplateLineStage_DailyJob.this,
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tS3Connection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:AgXRlYJXUQgqp+s6gYFW8vlH48+FRryEMAa1GglyX9fwFahjlIBbXrnO2yg2saKl1qnmS1O2eYxigB1NNII71So529c=");
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					com.amazonaws.services.s3.model.GetObjectRequest getObjectRequest_tS3Get_1 = new com.amazonaws.services.s3.model.GetObjectRequest(
							"libreriapacifico/raw", "saleOrderTemplateLine_20221031.csv");
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					com.amazonaws.services.s3.model.GetObjectRequest getObjectRequest_tS3Get_2 = new com.amazonaws.services.s3.model.GetObjectRequest(
							"libreriapacifico/stage", "saleOrderTemplateLine.csv");
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				strReturn = new String(commonByteArray_PACIFICO_getSaleOrderTemplateLineStage_DailyJob, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_2.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_uom_qty", "row2", temp, ex_tFileInputDelimited_2),
											ex_tFileInputDelimited_2));
								}
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_2.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_uom_id", "row2", temp, ex_tFileInputDelimited_2),
											ex_tFileInputDelimited_2));
								}
...
///////////////////////		
							...
							if (row2.sequence != null) { //
								row_tLogRow_2[1] = String.valueOf(row2.sequence);

							} //

							if (row2.sale_order_template_id != null) { //
								row_tLogRow_2[2] = String.valueOf(row2.sale_order_template_id);

							} //

							if (row2.company_id != null) { //
								row_tLogRow_2[3] = String.valueOf(row2.company_id);

							} //

							if (row2.name != null) { //
								row_tLogRow_2[4] = String.valueOf(row2.name);

							} //

							if (row2.product_id != null) { //
								row_tLogRow_2[5] = String.valueOf(row2.product_id);

							} //

							if (row2.product_uom_qty != null) { //
								row_tLogRow_2[6] = String.valueOf(row2.product_uom_qty);

							} //

							if (row2.product_uom_id != null) { //
								row_tLogRow_2[7] = String.valueOf(row2.product_uom_id);

							} //

							if (row2.display_type != null) { //
								row_tLogRow_2[8] = String.valueOf(row2.display_type);
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_uom_qty", "row1", temp, ex_tFileInputDelimited_1),
											ex_tFileInputDelimited_1));
								}
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_uom_id", "row1", temp, ex_tFileInputDelimited_1),
											ex_tFileInputDelimited_1));
								}
...
///////////////////////		
							...
							if (row1.sequence != null) { //
								row_tLogRow_1[1] = String.valueOf(row1.sequence);

							} //

							if (row1.sale_order_template_id != null) { //
								row_tLogRow_1[2] = String.valueOf(row1.sale_order_template_id);

							} //

							if (row1.company_id != null) { //
								row_tLogRow_1[3] = String.valueOf(row1.company_id);

							} //

							if (row1.name != null) { //
								row_tLogRow_1[4] = String.valueOf(row1.name);

							} //

							if (row1.product_id != null) { //
								row_tLogRow_1[5] = String.valueOf(row1.product_id);

							} //

							if (row1.product_uom_qty != null) { //
								row_tLogRow_1[6] = String.valueOf(row1.product_uom_qty);

							} //

							if (row1.product_uom_id != null) { //
								row_tLogRow_1[7] = String.valueOf(row1.product_uom_id);

							} //

							if (row1.display_type != null) { //
								row_tLogRow_1[8] = String.valueOf(row1.display_type);
...
///////////////////////    			
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = getSaleOrderTemplateLineStage_DailyJob.class.getClassLoader()
					.getResourceAsStream("pacifico/getsaleordertemplatelinestage_dailyjob_0_1/contexts/" + contextStr
							+ ".properties");
			if (inContext == null) {
				inContext = getSaleOrderTemplateLineStage_DailyJob.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : getSaleOrderTemplateLineStage_DailyJob");
		}
...
/************************************************************************************************
 * 124755 characters generated by Talend Open Studio for Data Integration on the
 * December 5, 2022 at 10:38:38 PM CST
 ************************************************************************************************/