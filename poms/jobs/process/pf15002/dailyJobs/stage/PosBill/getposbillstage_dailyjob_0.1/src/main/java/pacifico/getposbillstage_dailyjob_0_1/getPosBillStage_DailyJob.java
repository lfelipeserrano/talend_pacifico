		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tS3Connection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:tWSvNI62bS56nvoR+EcMKE8L+BF+hOxQTmWCeAWdux02NkYqygohat4u9MKmMAd8hf3FfUwkYPQr4abQSnfqbBXqEDI=");
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					com.amazonaws.services.s3.model.GetObjectRequest getObjectRequest_tS3Get_1 = new com.amazonaws.services.s3.model.GetObjectRequest(
							"libreriapacifico/raw", "posBill_20221031.csv");
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					com.amazonaws.services.s3.model.GetObjectRequest getObjectRequest_tS3Get_2 = new com.amazonaws.services.s3.model.GetObjectRequest(
							"libreriapacifico/stage", "posBill.csv");
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_2.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"value", "row2", temp, ex_tFileInputDelimited_2),
...
///////////////////////		
							...
							if (row2.name != null) { //
								row_tLogRow_2[1] = FormatterUtils.formatUnwithE(row2.name);

							} //

							if (row2.value != null) { //
								row_tLogRow_2[2] = String.valueOf(row2.value);

							} //

							if (row2.create_uid != null) { //
								row_tLogRow_2[3] = String.valueOf(row2.create_uid);

							} //

							if (row2.create_date != null) { //
								row_tLogRow_2[4] = FormatterUtils.format_Date(row2.create_date, "dd-MM-yyyy");

							} //

							if (row2.write_uid != null) { //
								row_tLogRow_2[5] = String.valueOf(row2.write_uid);

							} //

							if (row2.write_date != null) { //
								row_tLogRow_2[6] = FormatterUtils.format_Date(row2.write_date, "dd-MM-yyyy");
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"value", "row1", temp, ex_tFileInputDelimited_1),
...
///////////////////////		
							...
							if (row1.name != null) { //
								row_tLogRow_1[1] = FormatterUtils.formatUnwithE(row1.name);

							} //

							if (row1.value != null) { //
								row_tLogRow_1[2] = String.valueOf(row1.value);

							} //

							if (row1.create_uid != null) { //
								row_tLogRow_1[3] = String.valueOf(row1.create_uid);

							} //

							if (row1.create_date != null) { //
								row_tLogRow_1[4] = FormatterUtils.format_Date(row1.create_date, "dd-MM-yyyy");

							} //

							if (row1.write_uid != null) { //
								row_tLogRow_1[5] = String.valueOf(row1.write_uid);

							} //

							if (row1.write_date != null) { //
								row_tLogRow_1[6] = FormatterUtils.format_Date(row1.write_date, "dd-MM-yyyy");
...
///////////////////////    			
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = getPosBillStage_DailyJob.class.getClassLoader().getResourceAsStream(
					"pacifico/getposbillstage_dailyjob_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = getPosBillStage_DailyJob.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : getPosBillStage_DailyJob");
		}
...
/************************************************************************************************
 * 101729 characters generated by Talend Open Studio for Data Integration on the
 * December 5, 2022 at 10:34:35 PM CST
 ************************************************************************************************/