...
public class SalesFact implements TalendJob {
	...
	public void tAdvancedHash_saleOrderLine_error(Exception exception, String errorComponent,
			final java.util.Map<String, Object> globalMap) throws TalendException {
...
// # Lookup's keys initialization
				...
				org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<saleOrderLineStruct> tHash_Lookup_saleOrderLine = (org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<saleOrderLineStruct>) ((org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<saleOrderLineStruct>) globalMap
						.get("tHash_Lookup_saleOrderLine"));

				saleOrderLineStruct saleOrderLineHashKey = new saleOrderLineStruct();
				saleOrderLineStruct saleOrderLineDefault = new saleOrderLineStruct();
// ###############################        
...
// Start of branch "PosOrderLine"
						if (PosOrderLine != null) {
							...
							while ((tHash_Lookup_DimDate != null && tHash_Lookup_DimDate.hasNext())
									|| forceLoopDimDate) { // G_TM_M_043

								// CALL close loop of lookup 'DimDate'

								DimDateStruct fromLookup_DimDate = null;
								DimDate = DimDateDefault;

								if (!forceLoopDimDate) { // G 46
									...
									while ((tHash_Lookup_DimStore != null && tHash_Lookup_DimStore.hasNext())
										...
										while ((tHash_Lookup_DimCustomer != null && tHash_Lookup_DimCustomer.hasNext())
											...
											if (!rejectedInnerJoin_tMap_1) { // G_TM_M_020
												...
												DimProductHashKey.product_id = PosOrderLine.product_id;
											...
											if (tHash_Lookup_DimProduct != null
													&& tHash_Lookup_DimProduct.getCount(DimProductHashKey) > 1) { // G
																													// 071

												// System.out.println("WARNING: UNIQUE MATCH is configured for the
												// lookup 'DimProduct' and it contains more one result from keys :
											...
											if (tHash_Lookup_DimProduct != null && tHash_Lookup_DimProduct.hasNext()) { // G
																														// 099
											...
											///////////////////////////////////////////////
											// Starting Lookup Table "saleOrderLine"
											///////////////////////////////////////////////

											boolean forceLoopsaleOrderLine = false;

											saleOrderLineStruct saleOrderLineObjectFromLookup = null;

											if (!rejectedInnerJoin_tMap_1) { // G_TM_M_020

												hasCasePrimitiveKeyWithNull_tMap_1 = false;

												saleOrderLineHashKey.sale_order_line_id = PosOrderLine.sale_order_line_id;

												saleOrderLineHashKey.order_partner_id = DimCustomer.customer_id;

												saleOrderLineHashKey.hashCodeDirty = true;

												tHash_Lookup_saleOrderLine.lookup(saleOrderLineHashKey);

												if (!tHash_Lookup_saleOrderLine.hasNext()) { // G_TM_M_090
											...
											if (tHash_Lookup_saleOrderLine != null
													&& tHash_Lookup_saleOrderLine.getCount(saleOrderLineHashKey) > 1) { // G
																														// 071

												// System.out.println("WARNING: UNIQUE MATCH is configured for the
												// lookup 'saleOrderLine' and it contains more one result from keys :
												// saleOrderLine.sale_order_line_id = '" +
												// saleOrderLineHashKey.sale_order_line_id + "',
												// saleOrderLine.order_partner_id = '" +
												// saleOrderLineHashKey.order_partner_id + "'");
											} // G 071

											saleOrderLineStruct saleOrderLine = null;

											saleOrderLineStruct fromLookup_saleOrderLine = null;
											saleOrderLine = saleOrderLineDefault;

											if (tHash_Lookup_saleOrderLine != null
													&& tHash_Lookup_saleOrderLine.hasNext()) { // G 099

												fromLookup_saleOrderLine = tHash_Lookup_saleOrderLine.next();

											} // G 099

											if (fromLookup_saleOrderLine != null) {
												saleOrderLine = fromLookup_saleOrderLine;
											}
...
///////////////////////    			
							...
							} // close loop of lookup 'DimDate' // G_TM_M_043
...
// # Lookup hashes releasing
				...
				if (tHash_Lookup_saleOrderLine != null) {
					tHash_Lookup_saleOrderLine.endGet();
				}
				globalMap.remove("tHash_Lookup_saleOrderLine");
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_1 -
				// inputs:(PosOrderLine,DimProduct,DimCustomer,DimStore,DimPayment,PosOrder,DimDate,saleOrderLine)
				// outputs:(SalesFact)
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_id", "DimProduct", temp, ex_tFileInputDelimited_1),
											ex_tFileInputDelimited_1));
								}
							...
							DimProduct.weight = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									DimCustomer.customer_id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_2.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"customer_id", "DimCustomer", temp, ex_tFileInputDelimited_2),
											ex_tFileInputDelimited_2));
								}

							} else {

								DimCustomer.customer_id = null;
...
// Start of branch "DimCustomer"
						if (DimCustomer != null) {
							...
							DimCustomer_HashRow.customer_id = DimCustomer.customer_id;
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									rowstate_tFileInputDelimited_3.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"store_id", "DimStore", temp, ex_tFileInputDelimited_3),
											ex_tFileInputDelimited_3));
								}
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									DimPayment.payment_method_id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_4.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"payment_method_id", "DimPayment", temp, ex_tFileInputDelimited_4),
											ex_tFileInputDelimited_4));
								}

							} else {

								DimPayment.payment_method_id = null;
...
// Start of branch "DimPayment"
						if (DimPayment != null) {
							...
							DimPayment_HashRow.payment_method_id = DimPayment.payment_method_id;
	...
	public static class DimDateStruct implements routines.system.IPersistableRow<DimDateStruct> {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PACIFICO_SalesFact) {
				...
				try {
					...
					this.DateNum = readInteger(dis);

					this.Date = readDate(dis);

					this.YearMonthNum = readInteger(dis);

					this.Calendar_Quarter = readString(dis);

					this.MonthNum = readInteger(dis);

					this.MonthName = readString(dis);

					this.MonthShortName = readString(dis);
					...
					this.DayShortName = readString(dis);

					this.Quarter = readInteger(dis);

					this.YearQuarterNum = readInteger(dis);

					this.DayNumOfQuarter = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.Calendar_Quarter, dos);
				...
				writeString(this.MonthShortName, dos);
				...
				writeString(this.DayShortName, dos);
				...
				writeInteger(this.YearQuarterNum, dos);

				// Integer

				writeInteger(this.DayNumOfQuarter, dos);
	...
	public void tFileInputDelimited_7Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_DimDate = org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE.ALL_ROWS;
...
// Start of branch "DimDate"
	...
	public static class saleOrderLineStruct
			implements routines.system.IPersistableComparableLookupRow<saleOrderLineStruct> {
		final static byte[] commonByteArrayLock_PACIFICO_SalesFact = new byte[0];
		static byte[] commonByteArray_PACIFICO_SalesFact = new byte[0];
		protected static final int DEFAULT_HASHCODE = 1;
		protected static final int PRIME = 31;
		...
		public String price_reduce_taxinc;

		public String getPrice_reduce_taxinc() {
			return this.price_reduce_taxinc;
		}

		public String price_reduce_taxexcl;

		public String getPrice_reduce_taxexcl() {
		...
		public String product_uom_qty;

		public String getProduct_uom_qty() {
		...
		public String qty_delivered_manual;

		public String getQty_delivered_manual() {
		...
		public String untaxed_amount_invoiced;

		public String getUntaxed_amount_invoiced() {
			return this.untaxed_amount_invoiced;
		}

		public String untaxed_amount_to_invoice;

		public String getUntaxed_amount_to_invoice() {
		...
		public String customer_lead;

		public String getCustomer_lead() {
		...
		public String product_packaging_id;

		public String getProduct_packaging_id() {
			return this.product_packaging_id;
		}

		public String product_packaging_qty;

		public String getProduct_packaging_qty() {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + ((this.sale_order_line_id == null) ? 0 : this.sale_order_line_id.hashCode());

				result = prime * result + ((this.order_partner_id == null) ? 0 : this.order_partner_id.hashCode());
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final saleOrderLineStruct other = (saleOrderLineStruct) obj;

			if (this.sale_order_line_id == null) {
				if (other.sale_order_line_id != null)
					return false;

			} else if (!this.sale_order_line_id.equals(other.sale_order_line_id))

				return false;

			if (this.order_partner_id == null) {
				if (other.order_partner_id != null)
					return false;

			} else if (!this.order_partner_id.equals(other.order_partner_id))
		...
		public void copyDataTo(saleOrderLineStruct other) {
		...
		public void copyKeysDataTo(saleOrderLineStruct other) {
		...
		public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
			try {
				...
				this.price_unit = readString(dis, ois);

				this.price_subtotal = readString(dis, ois);

				this.price_tax = readString(dis, ois);

				this.price_total = readString(dis, ois);

				this.price_reduce = readString(dis, ois);

				this.price_reduce_taxinc = readString(dis, ois);

				this.price_reduce_taxexcl = readString(dis, ois);

				this.discount = readString(dis, ois);

				this.product_id = readInteger(dis, ois);

				this.product_uom_qty = readString(dis, ois);

				this.product_uom = readString(dis, ois);

				this.qty_delivered_method = readString(dis, ois);

				this.qty_delivered = readString(dis, ois);

				this.qty_delivered_manual = readString(dis, ois);

				this.qty_to_invoice = readString(dis, ois);

				this.qty_invoiced = readString(dis, ois);

				this.untaxed_amount_invoiced = readString(dis, ois);

				this.untaxed_amount_to_invoice = readString(dis, ois);

				this.salesman_id = readString(dis, ois);

				this.currency_id = readString(dis, ois);

				this.company_id = readString(dis, ois);

				this.is_expense = readString(dis, ois);

				this.is_downpayment = readString(dis, ois);

				this.state = readString(dis, ois);

				this.customer_lead = readString(dis, ois);

				this.display_type = readString(dis, ois);

				this.product_packaging_id = readString(dis, ois);

				this.product_packaging_qty = readString(dis, ois);
		...
		public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
			try {
				...
				this.price_unit = readString(dis, objectIn);

				this.price_subtotal = readString(dis, objectIn);

				this.price_tax = readString(dis, objectIn);

				this.price_total = readString(dis, objectIn);

				this.price_reduce = readString(dis, objectIn);

				this.price_reduce_taxinc = readString(dis, objectIn);

				this.price_reduce_taxexcl = readString(dis, objectIn);

				this.discount = readString(dis, objectIn);

				this.product_id = readInteger(dis, objectIn);

				this.product_uom_qty = readString(dis, objectIn);

				this.product_uom = readString(dis, objectIn);

				this.qty_delivered_method = readString(dis, objectIn);

				this.qty_delivered = readString(dis, objectIn);

				this.qty_delivered_manual = readString(dis, objectIn);

				this.qty_to_invoice = readString(dis, objectIn);

				this.qty_invoiced = readString(dis, objectIn);

				this.untaxed_amount_invoiced = readString(dis, objectIn);

				this.untaxed_amount_to_invoice = readString(dis, objectIn);

				this.salesman_id = readString(dis, objectIn);

				this.currency_id = readString(dis, objectIn);

				this.company_id = readString(dis, objectIn);

				this.is_expense = readString(dis, objectIn);

				this.is_downpayment = readString(dis, objectIn);

				this.state = readString(dis, objectIn);

				this.customer_lead = readString(dis, objectIn);

				this.display_type = readString(dis, objectIn);

				this.product_packaging_id = readString(dis, objectIn);

				this.product_packaging_qty = readString(dis, objectIn);
		...
		public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
			try {
				...
				writeString(this.price_unit, dos, oos);

				writeString(this.price_subtotal, dos, oos);

				writeString(this.price_tax, dos, oos);

				writeString(this.price_total, dos, oos);

				writeString(this.price_reduce, dos, oos);

				writeString(this.price_reduce_taxinc, dos, oos);

				writeString(this.price_reduce_taxexcl, dos, oos);

				writeString(this.discount, dos, oos);

				writeInteger(this.product_id, dos, oos);

				writeString(this.product_uom_qty, dos, oos);

				writeString(this.product_uom, dos, oos);

				writeString(this.qty_delivered_method, dos, oos);

				writeString(this.qty_delivered, dos, oos);

				writeString(this.qty_delivered_manual, dos, oos);

				writeString(this.qty_to_invoice, dos, oos);

				writeString(this.qty_invoiced, dos, oos);

				writeString(this.untaxed_amount_invoiced, dos, oos);

				writeString(this.untaxed_amount_to_invoice, dos, oos);

				writeString(this.salesman_id, dos, oos);

				writeString(this.currency_id, dos, oos);

				writeString(this.company_id, dos, oos);

				writeString(this.is_expense, dos, oos);

				writeString(this.is_downpayment, dos, oos);

				writeString(this.state, dos, oos);

				writeString(this.customer_lead, dos, oos);

				writeString(this.display_type, dos, oos);

				writeString(this.product_packaging_id, dos, oos);

				writeString(this.product_packaging_qty, dos, oos);
		...
		public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut) {
			try {
				...
				writeString(this.price_unit, dos, objectOut);

				writeString(this.price_subtotal, dos, objectOut);

				writeString(this.price_tax, dos, objectOut);

				writeString(this.price_total, dos, objectOut);

				writeString(this.price_reduce, dos, objectOut);

				writeString(this.price_reduce_taxinc, dos, objectOut);

				writeString(this.price_reduce_taxexcl, dos, objectOut);

				writeString(this.discount, dos, objectOut);

				writeInteger(this.product_id, dos, objectOut);

				writeString(this.product_uom_qty, dos, objectOut);

				writeString(this.product_uom, dos, objectOut);

				writeString(this.qty_delivered_method, dos, objectOut);

				writeString(this.qty_delivered, dos, objectOut);

				writeString(this.qty_delivered_manual, dos, objectOut);

				writeString(this.qty_to_invoice, dos, objectOut);

				writeString(this.qty_invoiced, dos, objectOut);

				writeString(this.untaxed_amount_invoiced, dos, objectOut);

				writeString(this.untaxed_amount_to_invoice, dos, objectOut);

				writeString(this.salesman_id, dos, objectOut);

				writeString(this.currency_id, dos, objectOut);

				writeString(this.company_id, dos, objectOut);

				writeString(this.is_expense, dos, objectOut);

				writeString(this.is_downpayment, dos, objectOut);

				writeString(this.state, dos, objectOut);

				writeString(this.customer_lead, dos, objectOut);

				writeString(this.display_type, dos, objectOut);

				writeString(this.product_packaging_id, dos, objectOut);

				writeString(this.product_packaging_qty, dos, objectOut);
		...
		public String toString() {
			...
			sb.append(",price_subtotal=" + price_subtotal);
			sb.append(",price_tax=" + price_tax);
			sb.append(",price_total=" + price_total);
			sb.append(",price_reduce=" + price_reduce);
			sb.append(",price_reduce_taxinc=" + price_reduce_taxinc);
			sb.append(",price_reduce_taxexcl=" + price_reduce_taxexcl);
			sb.append(",discount=" + discount);
			sb.append(",product_id=" + String.valueOf(product_id));
			sb.append(",product_uom_qty=" + product_uom_qty);
			sb.append(",product_uom=" + product_uom);
			sb.append(",qty_delivered_method=" + qty_delivered_method);
			sb.append(",qty_delivered=" + qty_delivered);
			sb.append(",qty_delivered_manual=" + qty_delivered_manual);
			sb.append(",qty_to_invoice=" + qty_to_invoice);
			sb.append(",qty_invoiced=" + qty_invoiced);
			sb.append(",untaxed_amount_invoiced=" + untaxed_amount_invoiced);
			sb.append(",untaxed_amount_to_invoice=" + untaxed_amount_to_invoice);
			sb.append(",salesman_id=" + salesman_id);
			...
			sb.append(",is_expense=" + is_expense);
			sb.append(",is_downpayment=" + is_downpayment);
			sb.append(",state=" + state);
			sb.append(",customer_lead=" + customer_lead);
			sb.append(",display_type=" + display_type);
			sb.append(",product_packaging_id=" + product_packaging_id);
			sb.append(",product_packaging_qty=" + product_packaging_qty);
		 ...
		 * Compare keys
		 */
		public int compareTo(saleOrderLineStruct other) {

			int returnValue = -1;

			returnValue = checkNullsAndCompare(this.sale_order_line_id, other.sale_order_line_id);
			if (returnValue != 0) {
				return returnValue;
			}

			returnValue = checkNullsAndCompare(this.order_partner_id, other.order_partner_id);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public void tFileInputDelimited_8Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				saleOrderLineStruct saleOrderLine = new saleOrderLineStruct();

				/**
				 * [tAdvancedHash_saleOrderLine begin ] start
				 */

				ok_Hash.put("tAdvancedHash_saleOrderLine", false);
				start_Hash.put("tAdvancedHash_saleOrderLine", System.currentTimeMillis());

				currentComponent = "tAdvancedHash_saleOrderLine";

				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "saleOrderLine");
				}

				int tos_count_tAdvancedHash_saleOrderLine = 0;

				// connection name:saleOrderLine
				// source node:tFileInputDelimited_8 - inputs:(after_tFileInputDelimited_5)
				// outputs:(saleOrderLine,saleOrderLine) | target
				// node:tAdvancedHash_saleOrderLine - inputs:(saleOrderLine) outputs:()
				// linked node: tMap_1 -
				// inputs:(PosOrderLine,DimProduct,DimCustomer,DimStore,DimPayment,PosOrder,DimDate,saleOrderLine)
				// outputs:(SalesFact)

				org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_saleOrderLine = org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE.UNIQUE_MATCH;

				org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<saleOrderLineStruct> tHash_Lookup_saleOrderLine = org.talend.designer.components.lookup.memory.AdvancedMemoryLookup
						.<saleOrderLineStruct>getLookup(matchingModeEnum_saleOrderLine);

				globalMap.put("tHash_Lookup_saleOrderLine", tHash_Lookup_saleOrderLine);

				/**
				 * [tAdvancedHash_saleOrderLine begin ] stop
				 */
				...
				try {
					...
					Object filename_tFileInputDelimited_8 = "C:/Program Files (x86)/TOS_DI-8.0.1/studio/workspace/saleOrderLine.csv";
					...
					try {
						fid_tFileInputDelimited_8 = new org.talend.fileprocess.FileInputDelimited(
								"C:/Program Files (x86)/TOS_DI-8.0.1/studio/workspace/saleOrderLine.csv", "ISO-8859-15",
					...
					while (fid_tFileInputDelimited_8 != null && fid_tFileInputDelimited_8.nextRecord()) {
						...
						saleOrderLine = new saleOrderLineStruct();
						try {
							...
							saleOrderLine.sale_order_line_id = fid_tFileInputDelimited_8
							...
							if (temp.length() > 0) {
								...
								try {
									...
									saleOrderLine.id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_8.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"id", "saleOrderLine", temp, ex_tFileInputDelimited_8),
											ex_tFileInputDelimited_8));
								}

							} else {

								saleOrderLine.id = null;
							...
							if (temp.length() > 0) {
								...
								try {
									...
									saleOrderLine.order_id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_8.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"order_id", "saleOrderLine", temp, ex_tFileInputDelimited_8),
											ex_tFileInputDelimited_8));
								}

							} else {

								saleOrderLine.order_id = null;

							}

							columnIndexWithD_tFileInputDelimited_8 = 3;

							saleOrderLine.name = fid_tFileInputDelimited_8.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 4;

							saleOrderLine.sequence = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 5;

							saleOrderLine.invoice_status = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 6;

							saleOrderLine.price_unit = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 7;

							saleOrderLine.price_subtotal = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 8;

							saleOrderLine.price_tax = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 9;

							saleOrderLine.price_total = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 10;

							saleOrderLine.price_reduce = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 11;

							saleOrderLine.price_reduce_taxinc = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 12;

							saleOrderLine.price_reduce_taxexcl = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 13;

							saleOrderLine.discount = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);
							...
							if (temp.length() > 0) {
								...
								try {
									...
									saleOrderLine.product_id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_8.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"product_id", "saleOrderLine", temp, ex_tFileInputDelimited_8),
											ex_tFileInputDelimited_8));
								}

							} else {

								saleOrderLine.product_id = null;

							}

							columnIndexWithD_tFileInputDelimited_8 = 15;

							saleOrderLine.product_uom_qty = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 16;

							saleOrderLine.product_uom = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 17;

							saleOrderLine.qty_delivered_method = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 18;

							saleOrderLine.qty_delivered = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 19;

							saleOrderLine.qty_delivered_manual = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 20;

							saleOrderLine.qty_to_invoice = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 21;

							saleOrderLine.qty_invoiced = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 22;

							saleOrderLine.untaxed_amount_invoiced = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 23;

							saleOrderLine.untaxed_amount_to_invoice = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 24;

							saleOrderLine.salesman_id = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 25;

							saleOrderLine.currency_id = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 26;

							saleOrderLine.company_id = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);
							...
							if (temp.length() > 0) {
								...
								try {
									...
									saleOrderLine.order_partner_id = ParserUtils.parseTo_Integer(temp);
									...
									rowstate_tFileInputDelimited_8.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"order_partner_id", "saleOrderLine", temp, ex_tFileInputDelimited_8),
											ex_tFileInputDelimited_8));
								}

							} else {

								saleOrderLine.order_partner_id = null;

							}

							columnIndexWithD_tFileInputDelimited_8 = 28;

							saleOrderLine.is_expense = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 29;

							saleOrderLine.is_downpayment = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 30;

							saleOrderLine.state = fid_tFileInputDelimited_8.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 31;

							saleOrderLine.customer_lead = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 32;

							saleOrderLine.display_type = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 33;

							saleOrderLine.product_packaging_id = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 34;

							saleOrderLine.product_packaging_qty = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 35;

							saleOrderLine.create_uid = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 36;

							saleOrderLine.create_date = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 37;

							saleOrderLine.write_uid = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 38;

							saleOrderLine.write_date = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);

							columnIndexWithD_tFileInputDelimited_8 = 39;

							saleOrderLine.route_id = fid_tFileInputDelimited_8
									.get(columnIndexWithD_tFileInputDelimited_8);
						 ...
						 */
// Start of branch "saleOrderLine"
						if (saleOrderLine != null) {

							/**
							 * [tAdvancedHash_saleOrderLine main ] start
							 */
							...
							saleOrderLineStruct saleOrderLine_HashRow = new saleOrderLineStruct();

							saleOrderLine_HashRow.sale_order_line_id = saleOrderLine.sale_order_line_id;

							saleOrderLine_HashRow.id = saleOrderLine.id;

							saleOrderLine_HashRow.order_id = saleOrderLine.order_id;

							saleOrderLine_HashRow.name = saleOrderLine.name;

							saleOrderLine_HashRow.sequence = saleOrderLine.sequence;

							saleOrderLine_HashRow.invoice_status = saleOrderLine.invoice_status;

							saleOrderLine_HashRow.price_unit = saleOrderLine.price_unit;

							saleOrderLine_HashRow.price_subtotal = saleOrderLine.price_subtotal;

							saleOrderLine_HashRow.price_tax = saleOrderLine.price_tax;

							saleOrderLine_HashRow.price_total = saleOrderLine.price_total;

							saleOrderLine_HashRow.price_reduce = saleOrderLine.price_reduce;

							saleOrderLine_HashRow.price_reduce_taxinc = saleOrderLine.price_reduce_taxinc;

							saleOrderLine_HashRow.price_reduce_taxexcl = saleOrderLine.price_reduce_taxexcl;

							saleOrderLine_HashRow.discount = saleOrderLine.discount;

							saleOrderLine_HashRow.product_id = saleOrderLine.product_id;

							saleOrderLine_HashRow.product_uom_qty = saleOrderLine.product_uom_qty;

							saleOrderLine_HashRow.product_uom = saleOrderLine.product_uom;

							saleOrderLine_HashRow.qty_delivered_method = saleOrderLine.qty_delivered_method;

							saleOrderLine_HashRow.qty_delivered = saleOrderLine.qty_delivered;

							saleOrderLine_HashRow.qty_delivered_manual = saleOrderLine.qty_delivered_manual;

							saleOrderLine_HashRow.qty_to_invoice = saleOrderLine.qty_to_invoice;

							saleOrderLine_HashRow.qty_invoiced = saleOrderLine.qty_invoiced;

							saleOrderLine_HashRow.untaxed_amount_invoiced = saleOrderLine.untaxed_amount_invoiced;

							saleOrderLine_HashRow.untaxed_amount_to_invoice = saleOrderLine.untaxed_amount_to_invoice;

							saleOrderLine_HashRow.salesman_id = saleOrderLine.salesman_id;

							saleOrderLine_HashRow.currency_id = saleOrderLine.currency_id;

							saleOrderLine_HashRow.company_id = saleOrderLine.company_id;

							saleOrderLine_HashRow.order_partner_id = saleOrderLine.order_partner_id;

							saleOrderLine_HashRow.is_expense = saleOrderLine.is_expense;

							saleOrderLine_HashRow.is_downpayment = saleOrderLine.is_downpayment;

							saleOrderLine_HashRow.state = saleOrderLine.state;

							saleOrderLine_HashRow.customer_lead = saleOrderLine.customer_lead;

							saleOrderLine_HashRow.display_type = saleOrderLine.display_type;

							saleOrderLine_HashRow.product_packaging_id = saleOrderLine.product_packaging_id;

							saleOrderLine_HashRow.product_packaging_qty = saleOrderLine.product_packaging_qty;

							saleOrderLine_HashRow.create_uid = saleOrderLine.create_uid;

							saleOrderLine_HashRow.create_date = saleOrderLine.create_date;

							saleOrderLine_HashRow.write_uid = saleOrderLine.write_uid;

							saleOrderLine_HashRow.write_date = saleOrderLine.write_date;

							saleOrderLine_HashRow.route_id = saleOrderLine.route_id;

							tHash_Lookup_saleOrderLine.put(saleOrderLine_HashRow);

							tos_count_tAdvancedHash_saleOrderLine++;

							/**
							 * [tAdvancedHash_saleOrderLine main ] stop
							 */

							/**
							 * [tAdvancedHash_saleOrderLine process_data_begin ] start
							 */

							currentComponent = "tAdvancedHash_saleOrderLine";

							/**
							 * [tAdvancedHash_saleOrderLine process_data_begin ] stop
							 */

							/**
							 * [tAdvancedHash_saleOrderLine process_data_end ] start
							 */

							currentComponent = "tAdvancedHash_saleOrderLine";

							/**
							 * [tAdvancedHash_saleOrderLine process_data_end ] stop
							 */

						} // End of branch "saleOrderLine"
					...
					if (!((Object) ("C:/Program Files (x86)/TOS_DI-8.0.1/studio/workspace/saleOrderLine.csv") instanceof java.io.InputStream)) {
				...
				/**
				 * [tAdvancedHash_saleOrderLine end ] start
				 */

				currentComponent = "tAdvancedHash_saleOrderLine";

				tHash_Lookup_saleOrderLine.endPut();

				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "saleOrderLine");
				}

				ok_Hash.put("tAdvancedHash_saleOrderLine", true);
				end_Hash.put("tAdvancedHash_saleOrderLine", System.currentTimeMillis());

				/**
				 * [tAdvancedHash_saleOrderLine end ] stop
				 */
			...
			try {
				...
				/**
				 * [tAdvancedHash_saleOrderLine finally ] start
				 */

				currentComponent = "tAdvancedHash_saleOrderLine";

				/**
				 * [tAdvancedHash_saleOrderLine finally ] stop
				 */
...
/************************************************************************************************
 * 372064 characters generated by Talend Open Studio for Data Integration on the
 * December 6, 2022 at 11:23:26 PM CST
 ************************************************************************************************/