...
package pacifico.dailyjobsppt_template_0_1;
...
/**
 * Job: dailyJobsPpt_template Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class dailyJobsPpt_template implements TalendJob {
	...
	private final String jobName = "dailyJobsPpt_template";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					dailyJobsPpt_template.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(dailyJobsPpt_template.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_PACIFICO_dailyJobsPpt_template = new byte[0];
		static byte[] commonByteArray_PACIFICO_dailyJobsPpt_template = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PACIFICO_dailyJobsPpt_template.length) {
					if (length < 1024 && commonByteArray_PACIFICO_dailyJobsPpt_template.length == 0) {
						commonByteArray_PACIFICO_dailyJobsPpt_template = new byte[1024];
					} else {
						commonByteArray_PACIFICO_dailyJobsPpt_template = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PACIFICO_dailyJobsPpt_template, 0, length);
				strReturn = new String(commonByteArray_PACIFICO_dailyJobsPpt_template, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PACIFICO_dailyJobsPpt_template, 0, length);
				strReturn = new String(commonByteArray_PACIFICO_dailyJobsPpt_template, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PACIFICO_dailyJobsPpt_template) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tS3Connection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:sMrUtvDSopP1aMy9SnwZRP3nBLH+ZmF3GL53uliNRCKPgbGRLWsrtyCDQg+PCFMIvLG6e7PBWRGsGA9ZrI/pS2XDkIE=");
...
// Start of branch "productTemplate"
	...
	public static void main(String[] args) {
		final dailyJobsPpt_template dailyJobsPpt_templateClass = new dailyJobsPpt_template();

		int exitCode = dailyJobsPpt_templateClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = dailyJobsPpt_template.class.getClassLoader()
					.getResourceAsStream("pacifico/dailyjobsppt_template_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = dailyJobsPpt_template.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : dailyJobsPpt_template");
		}
...
/************************************************************************************************
 * 231454 characters generated by Talend Open Studio for Data Integration on the
 * November 2, 2022 at 6:33:35 PM CST
 ************************************************************************************************/